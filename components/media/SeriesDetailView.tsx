      ...
      {nextUpItems.length > 0 && (
        <View style={styles.sectionBlock}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <MediaCard item={item} style={styles.horizontalCard} imgType="Primary" />
            )}
...
function SeasonModeContent({ episodes, item }: { episodes: BaseItemDto[]; item: BaseItemDto }) {
...
function ItemMeta({ item }: { item: BaseItemDto }) {
  const textColor = useThemeColor({ light: '#000', dark: '#fff' }, 'text');
...
function ItemInfoList({ item }: { item: BaseItemDto }) {
  const subtitleColor = useThemeColor({ light: '#666', dark: '#999' }, 'text');
